...
# The database produced in R will have a slightly different name to differentiate it from the object that holds the citation text.
...
#################JUST IN CASE! # a complete script for this ("filter material citation")
#N. ### Adding... (to exclude material citation) - Archive 'c' for the time slot (i.e.1900_49c.RData)
...
dbs_mkd_taxa_material <- left_join(dbs_mkd_taxa_new, databases_basis, by = "id")
dbs_mkd_taxa_material <- filter(dbs_mkd_taxa_material, basisOfRecord != "MATERIAL_CITATION")
unique(dbs_mkd_taxa_material$basisOfRecord)
