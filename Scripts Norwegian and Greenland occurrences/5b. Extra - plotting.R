...
#last correction: masking with "world"
rm(i,x,a, mn, mx,setone,settwo,y)
for (i in 1:length(namesShallowPlot)){
  ...
  while (mn < 6000){
    ...
    x <- st_crop(world, xmin = 0, ymin = 0, xmax = 1, ymax = 1)
  ...
  assign(gsub(" ","",paste(namesShallowPlot[i], "_diffmatch2")),setone)  
  rm(x,y,a, setone, setwo)
  #rm(z)
}
  
namesShallowPlot2 <- gsub("_diffmatch", "_diffmatch_diffmatch2", namesShallowPlot)
