...
#II. Taxon filtering with WoRMS database: filtering out non-marine classes, families and species.
rm(b)
length(set24_mkd)

for (b in 1:length(set24_mkd){
...
#Reading information from WoRMS in order to determine which families are non-marine
...
# Run this part and add from new lists as it is time-convenient. These one-two lists are assessed with taxa from 1970to79
...
#Run this line only for GBIF and OBIS records. Exclude if the lenght of the logical argument is 0 (there is no records with individual counts recorded with minus values). 
